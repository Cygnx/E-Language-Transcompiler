...
public class Scan {
		...
		while (true) {
			...
			if (myisalpha((char) c)) {
				...
				case '#': // gobble comments
					...
					break;
				default:
					System.err.println("scan: line " + linenumber
							+ " bad char (ASCII " + c + ")");
					break;
				}
			}
	...
	private Token ccase1or2(char c1, char c2, TK r1, TK r2) {
		int c = getchar();
		if (c == c2) {
			return new Token(r2, new String(String.valueOf(c1)
					+ String.valueOf(c2)), linenumber);
		} else {
			putback = true;
	...
	private Token ccase2(char c1, char c2, TK r) {
		int c = getchar();
		if (c == c2) {
			return new Token(r, String.valueOf(c1) + String.valueOf(c2),
					linenumber);
		} else {
			System.err.println("scan: got got " + c1 + " missing " + c2
					+ " (got ASCII " + c + ")");
	...
	private String buildNUM() {
		...
		if (myisdigit((char) c) && k == MAXLEN_ID) {
			...
			System.err.println("scan: number too long -- truncated to " + str);
		}
		return str;
	}
